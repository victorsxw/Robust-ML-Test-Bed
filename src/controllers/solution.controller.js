...
const findSolutionByAttackId = (req, res) => {
    ...
    const Attack_ID = req.body.Attack_ID;
    console.log("Searching for Attack_ID:", Attack_ID);

    Solution.findByAttackId(Attack_ID, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
                    message: `No solution found for Attack_ID ${Attack_ID}`
                });
            } else {
                res.status(500).send({
                    message: "Error retrieving solution for Attack_ID " + Attack_ID
                });
            }
        } else {
            res.send(data);
